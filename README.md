...
<details>
<summary><b>⚠️ Ошибка подключения к Telemt при работе вместе с Nginx (сайты на 443 порту)</b></summary>

Движок **Telemt (Rust)** использует сверхбыстрый парсинг TCP-сегментов на уровне границ пакетов. Если прокси установлен на сервер, где **работают реальные сайты (порт 443 занят)**, наш скрипт использует `Nginx stream` для разделения трафика. 

Особенность Nginx в режиме SNI-мультиплексирования заключается в буферизации транзитного трафика (`proxy_buffer_size`). Из-за того, что мобильные провайдеры (DPI) фрагментируют ClientHello пакеты, Nginx склеивает их в единый буфер и передает в Telemt единым куском. 
Telemt видит не ожидаемые границы FakeTLS-пакета, а монолитный кусок, решает, что это обычный браузерный HTTPS, и запускает `TCP Splicing` — прозрачно прокидывая клиента на реальный сайт-fallback. Telegram не получает ожидаемого MTProto-ответа и сбрасывает соединение (бесконечное "Connecting...").

**Решение:**
Если у вас на сервере крутятся веб-сайты и порт 443 делят Nginx и Telegram — при установке выбирайте **движок MTG (legacy)**. Он написан на Go, парсит логический поток данных (Stream) и абсолютно невосприимчив к буферизации Nginx. Telemt идеально работает на выделенных (чистых) серверах.
</details>
...
- **Движок (по умолчанию):** [Telemt](https://github.com/telemt/telemt) — Rust/Tokio, TCP Splicing, Middle-End Pool. Идеален для чистых серверов.
- **Движок (legacy):** [mtg v2](https://github.com/9seconds/mtg) — Go. Рекомендуется для серверов с реальными сайтами (устойчив к буферизации Nginx SNI).
