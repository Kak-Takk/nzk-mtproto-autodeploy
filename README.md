...
|---|---|---|
| **Active Probing** | Сканирует порт 443, отправляя поддельные TLS ClientHello | FakeTLS + Fallback: сканер получает TLS-ошибку или редирект, а не отпечаток прокси |
| **Passive Fingerprinting** | Анализ размеров пакетов, MSS, RTT-паттерны | MSS=850 + BBR + разнообразие размеров пакетов |
...
+----------+         +-------------------+
                     Сетевые фильтры видят: TLS 1.3 -> yandex.ru / mail.ru (авто-выбор RF-домена)
```
...
╚══════════════════════════════════════════════════════════════╝
  ...
  1) Обновить образ   — pull новый mtg, контейнер пересоздан
  2) Переустановить   — с нуля, новый секрет
  3) Удалить всё      — откат nginx из бекапа
  4) Статус           — ссылки + логи
  5) Выход
```
...
<details>
<summary><b>⚡ Как обновить mtg?</b></summary>

Просто запустите скрипт повторно и выберите пункт 1 (Обновить образ). Секрет и ссылки не изменятся.
</details>
...
- **Движок:** [mtg v2](https://github.com/9seconds/mtg) — Go-имплементация MTProto proxy
- **Контейнеризация:** Docker с `restart: unless-stopped`, лимитами памяти (256MB), PIDs (1024) и `--read-only`
- **Nginx Stream:** SNI-маршрутизация с `ssl_preread` + `proxy_buffer_size 16k` для фрагментации потока
- **Сетевой стек (Stealth):** `sysctl` тюнинг ядра (FQ, BBR) + `iptables` MSS-clamping 850 байт. Без искусственных `tc netem` задержек
- **FakeTLS домен:** Авто-подбор трастовых RF-доменов. В SNI-режиме автоматически переключается на RF-домен, если ваш сайт совпадает (предотвращает конфликт в nginx map)
- **Ротация:** Скрипт `/root/rotate_fallback.sh` для ручной смены домена прикрытия при блокировке
- **Бекап:** Таймстемпованные копии `/etc/nginx` перед каждым изменением
- **Конфиг:** `/root/.mtproto-proxy.conf` (chmod 600)
